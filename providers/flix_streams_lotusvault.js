...
const PROVIDER_NAME = "Flix-Streams LotusVault";
...
function isLotusVaultStream(stream) {
  ...
  return /\blotus\s*vault\b/i.test(text)
    || /\blotusvault\b/i.test(text)
    || /\benable[-_]?lotus[-_]?vault\b/i.test(text);
}
...
async function getStreams(tmdbId, mediaType = "movie", season = null, episode = null, imdbId = null) {
  try {
    ...
    return streams.filter(isLotusVaultStream).map(normalizeFlixStream).filter(Boolean);
  } catch (error) {
