...
const PROVIDER_NAME = "Flix-Streams Stream Vault";
...
function isStreamVaultStream(stream) {
  const text = flixText(stream);
  return /\bstream\s*vault\b/i.test(text)
    || /\bstreamvault\b/i.test(text)
    || /\benable[-_]?stream[-_]?vault\b/i.test(text)
    || /\/api\/stream[-_]?vault\/media\b/i.test(text);
}
...
async function getStreams(tmdbId, mediaType = "movie", season = null, episode = null, imdbId = null) {
  try {
    ...
    return streams.filter(isStreamVaultStream).map(normalizeFlixStream).filter(Boolean);
  } catch (error) {
